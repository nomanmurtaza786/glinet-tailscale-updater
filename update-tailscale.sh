...
#
#
# Description: This script updates tailscale on GL.iNet routers to a specific version
# Thread: https://forum.gl-inet.com/t/how-to-update-tailscale-on-arm64/37582
# Author: Admon
# Contributor: lwbt
# Date: 2024-01-24
SCRIPT_VERSION="2025.04.23.01"
SCRIPT_NAME="install-specific-tailscale.sh"
UPDATE_URL="https://raw.githubusercontent.com/Admonstrator/glinet-tailscale-updater/main/update-tailscale.sh"
TAILSCALE_BASE_URL="https://pkgs.tailscale.com/stable/"
TAILSCALE_TINY_URL="https://github.com/Admonstrator/glinet-tailscale-updater/releases/download/"
SPECIFIC_VERSION="1.78.0" # Specify the desired Tailscale version here
#
# Usage: ./install-specific-tailscale.sh [--ignore-free-space] [--force] [--restore] [--no-upx] [--no-download] [--no-tiny] [--help]
...
invoke_intro() {
    ...
    echo "| Installing Tailscale version: $SPECIFIC_VERSION                         |"
    echo "├────────────────────────────────────────────────────────────────────────┤"
...
\}
preflight\_check\(\) \{
AVAILABLE\_SPACE\=</span>(df -k / | tail -n 1 | awk '{print <span class="math-inline">4/1024\}'\)
AVAILABLE\_SPACE\=</span>(printf "%.0f" "<span class="math-inline">AVAILABLE\_SPACE"\)
ARCH\=</span>(uname -m)
    FIRMWARE_VERSION=<span class="math-inline">\(cut \-c1 </etc/glversion\)
PREFLIGHT\=0
TINY\_ARCH\=""
log "INFO" "Checking if prerequisites are met"
if \[ "</span>{FIRMWARE_VERSION}" -lt 4 ]; then
...
MODEL\=</span>(grep 'machine' /proc/cpuinfo | awk -F ': ' '{print $2}')
    ...
    if [ "<span class="math-inline">PREFLIGHT" \-eq "1" \]; then
...
get_specific_tailscale_version_tiny() {
    # Will attempt to download the specific version of tiny tailscale
    log "INFO" "Downloading tiny tailscale version: $SPECIFIC_VERSION"
    local filename="tailscaled-linux-$TINY_ARCH"
    local download_url="$TAILSCALE_TINY_URL/$filename"
    log "INFO" "Downloading from: $download_url"
    curl -L -s --output /tmp/$filename "$download_url"

    # Check if download was successful
    if [ ! -f "/tmp/$filename" ]; then
        log "ERROR" "Could not download tailscale version $SPECIFIC_VERSION (tiny). Exiting"
        log "ERROR" "File not found: /tmp/$filename"
        exit 1
    fi
    log "SUCCESS" "Successfully downloaded tiny tailscale version $SPECIFIC_VERSION"
}

get_specific_tailscale_version() {
    ...
    if [ "$NO_DOWNLOAD" -eq 1 ]; then
        ...
        log "INFO" "Downloading tailscale version: $SPECIFIC_VERSION"
        local filename=""
        if [ "<span class="math-inline">ARCH" \= "aarch64" \]; then
filename\="tailscale\_</span>{SPECIFIC_VERSION}_arm64.tgz"
        elif [ "<span class="math-inline">ARCH" \= "armv7l" \]; then
filename\="tailscale\_</span>{SPECIFIC_VERSION}_arm.tgz"
        elif [ "<span class="math-inline">ARCH" \= "mips" \]; then
filename\="tailscale\_</span>{SPECIFIC_VERSION}_mips.tgz"
        fi
        local download_url="$TAILSCALE_BASE_URL/$filename"
        log "INFO" "Downloading from: $download_url"
        curl -L -s --output /tmp/tailscale.tar.gz "$download_url"

        # Check if download was successful
        if [ ! -f "/tmp/tailscale.tar.gz" ]; then
            log "ERROR" "Could not download tailscale version $SPECIFIC_VERSION. Exiting"
...
TAILSCALE\_SUBDIR\_IN\_TAR\=</span>(tar tzf /tmp/tailscale.tar.gz | grep /$ | head -n 1)
    ...
    if [ "<span class="math-inline">answer\_compress\_binaries" \!\= "</span>{answer_compress_binaries#[Yy]}" ]; then
        compress_binaries
        if [ "<span class="math-inline">UPX\_ERROR" \-eq 1 \]; then
...
"https\://github\.com/upx/upx/releases/download/v</span>{upx_version}/upx-<span class="math-inline">\{upx\_version\}\-</span>{UPX_ARCH}_linux.tar.xz"
    ...
    if [ ! -f "/tmp/upx.tar.xz" ]; then
        ...
        unxz --decompress --stdout "/tmp/upx.tar.xz" |
            tar x -C "/tmp/" "upx-<span class="math-inline">\{upx\_version\}\-</span>{UPX_ARCH}_linux/upx"
        mv "/tmp/upx-<span class="math-inline">\{upx\_version\}\-</span>{UPX_ARCH}_linux/upx" "/tmp/upx"
        rmdir "/tmp/upx-<span class="math-inline">\{upx\_version\}\-</span>{UPX_ARCH}_linux"
        rm "/tmp/upx.tar.xz"
